...
from agent.store.inMemoryStore40 import format_memories_for_prompt
...
- unit_converter: 单位转换
- memory_save: 保存长期记忆（当用户提到重要信息时自动保存）
- memory_search: 搜索长期记忆（当需要回忆用户信息时使用）

请根据用户的问题，决定是否需要使用工具。如果需要使用工具，请调用相应的工具；
如果不需要工具，直接回答用户的问题即可。

当用户提到个人偏好、习惯、重要事实等信息时，请主动使用 memory_save 工具保存。
当需要回忆用户之前提到的信息时，请使用 memory_search 工具搜索。
...
def agent_node(state: AgentState) -> dict:
    ...
    2. 注入长期记忆到系统提示
    3. 调用 LLM（带工具绑定）
    4. 返回 LLM 的响应（可能包含工具调用）
    """
    messages = state["messages"]

    # 构建系统提示（注入长期记忆）
    system_prompt = SYSTEM_PROMPT
    memory_text = format_memories_for_prompt("default", limit=5)
    if memory_text:
